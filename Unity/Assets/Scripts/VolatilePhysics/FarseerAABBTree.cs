...
namespace FarseerPhysics.Collision
{
  ...
  public class DynamicTree<T>
  {
    ...
    public int AddProxy(AABB aabb, T userData)
    {
      ...
      // Fatten the aabb.
      _nodes[proxyId].AABB = AABB.CreateExpanded(aabb, 0.2f);
    ...
    private void InsertLeaf(int leaf)
    {
      ...
      while (_nodes[index].IsLeaf() == false)
      {
        ...
        combinedAABB = AABB.CreateMerged(_nodes[index].AABB, leafAABB);
        ...
        {
          AABB aabb = new AABB();
          aabb = AABB.CreateMerged(leafAABB, _nodes[child1].AABB);
        ...
        {
          AABB aabb = new AABB();
          aabb = AABB.CreateMerged(leafAABB, _nodes[child2].AABB);
      ...
      _nodes[newParent].AABB = AABB.CreateMerged(leafAABB, _nodes[sibling].AABB);
      ...
      while (index != NullNode)
      {
        ...
        _nodes[index].AABB = AABB.CreateMerged(_nodes[child1].AABB, _nodes[child2].AABB);
    ...
    public void ValidateMetrics(int index)
    {
      ...
      AABB AABB = new AABB();
      AABB = AABB.CreateMerged(_nodes[child1].AABB, _nodes[child2].AABB);
    ...
    {
      this.DoGizmoDraw(aabbColor, this._root);
    }
    ...
    private void DoGizmoDraw(Color color, int nodeId)
    {
      ...
      this.DoGizmoDraw(color, node.Child1);
      this.DoGizmoDraw(color, node.Child2);
    }
